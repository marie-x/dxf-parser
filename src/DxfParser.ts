...
import BoundaryPath from './entities/subentities/boundarypath';
...
export interface IStyle {
	...
	obliqueAngle: number;
	textGenerationFlags: number; // unsure about this . . . KR
...
function registerDefaultEntityHandlers(dxfParser: DxfParser) {
	...
	dxfParser.registerEntityHandler(BoundaryPath);
			...
			while (!groupIs(curr, 0, 'ENDTAB')) {
				switch (curr.code) {
					...
					case 50:
						...
						break;
					case 71:
						style.textGenerationFlags = curr.value as number; // Place holder. Need to figure out how to parse
						curr = scanner.next();
						break;
					case 3:
						style.primaryFontName = curr.value as string;
						curr = scanner.next();
						break;
					case 4:
						style.bigFontName = curr.value as string;
						curr = scanner.next();
						break;
					case 1071:
						style.fontDetails = curr.value as string;
						curr = scanner.next();
						break;
					case 0:
						// New Style
